...
import cv2
import csv
csvfile =  open("dance1.csv","w", newline='')
...
def track(cap,video):
    ...
    order = [[],[],[],[]]
    condition = True
    old_red = [(509, 361), (540, 387)]
    old_blue = [(509, 361), (540, 387)]
    old_yellow = [(509, 361), (540, 387)]
    old_green = [(509, 361), (540, 387)]

    while condition:   
        _,img = cap.read()
        if args.get("video") and not _:
            break
        ...
        for pic, contour in enumerate(contours):
            ...
            if(area>300):
                ...
                old_blue = [(x,y),(x+w,y+h)]
                img = cv2.rectangle(img,(x,y),(x+w,y+h),(0,0,255),2)
                # cv2.putText(img,"Blue color",(x,y),cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255,0,0))
            else:
                # print ('blue',ld_blue)
                if old_blue != [[]]:    
                    order[1].append(old_blue)
        ...
        for pic, contour in enumerate(contours):
            ...
            if(area>300):
                ...
                old_green = [(x,y),(x+w,y+h)]
                img = cv2.rectangle(img,(x,y),(x+w,y+h),(0,255,0),2)
            else:
                # print ('green',old_green)
                if old_green != [[]]:
                    order[3].append(old_green)    
